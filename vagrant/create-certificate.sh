...
PATH_ROOT_CRT="${PATH_SSL}/ca.${SSLHOST}.crt"
...
"
    ...
    openssl req -config "$PATH_ROOT_CNF" \
        -key "$PATH_ROOT_KEY" \
        -x509 -new -extensions v3_ca -days $SSLDays -sha256 \
...
"
    ...
    openssl x509 -req -extfile "$PATH_CNF" \
        -extensions server_cert -days $SSLDays -sha256 \
        -in "$PATH_CSR" \
